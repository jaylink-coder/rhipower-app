...
// something to run VAT/discount/bulk math on yet. That reconciliation only
// makes sense once a delivery invoice actually exists (see ReceiveLineRow),
// and any resulting correction to an item's real buying price happens on
// the Add/Edit Item page, not here.
...
function ReceiveLineRow({ po, line, item, session, business, onReceived }) {
  ...
  // calculator) and correct buying_price_kes there. Receiving itself just
  // records the movement at the planned cost.
  const priceMismatch = item?.buying_price_kes != null && Number(item.buying_price_kes) !== Number(line.unit_cost_kes)
        ...
        <div className="flex-1 min-w-0">
          ...
          {priceMismatch && (
            <div className="text-[11px] text-amber-600 mt-0.5">
              ⚠️ Item's current buying price ({formatKsh(item.buying_price_kes)}) differs from this PO's planned cost. If the delivery invoice confirms a different price, correct it on the item's Edit page.
            </div>
          )}
        </div>
        {canReceive ? (
